...
int main() {
  //INT DECLARATIONS  
  int choice, playerHandValue, dealerHandValue, playerIndexPos, dealerIndexPos, exitFlag, i, newCard, wincon=0, funds, bet, playAgain=1;
  srand(time(0));
  ...
  while (playAgain = 1) { //MAIN GAME LOOP
    ...
    while (fgets(deck[i].name, sizeof(deck[i].name), blackjackDeck)) {
      deck[i].name[strcspn(deck[i].name, "\r\n")] = 0;;  //removes newline character
      i++;
    }
    fclose(blackjackDeck);
